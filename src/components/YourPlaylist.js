...
function YourPlaylist({ onClose }) {
  ...
  const handlePlaylistClick = async (playlistId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://3.18.215.126:8082/api/playlists/tracks/${playlistId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleDeletePlaylist = async (playlistId) => {
    try {
      ...
      await axios.delete(`http://3.18.215.126:8082/api/playlists/${playlistId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleDeleteTrack = async (trackId, playlistId) => {
    try {
      ...
      await axios.delete(`http://3.18.215.126:8082/api/deleteTrack`, {
        headers: {
          Authorization: `Bearer ${token}`
        },
        data: {
